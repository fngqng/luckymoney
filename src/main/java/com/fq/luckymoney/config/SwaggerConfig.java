...
public class SwaggerConfig {
    ...
    public Docket controllerApi(){
        ...
        if (!StringUtils.isEmpty(swaggerInfo.getBasePackage())){
            //根据包选择 UserController.class.getPackege().getName
    ...
    private ApiInfo apiInfo(){
        return new ApiInfoBuilder().title(swaggerInfo.getTitle()).description(swaggerInfo.getDescription()).termsOfServiceUrl(swaggerInfo.getServiceUrl()).license(swaggerInfo.getLicense()).licenseUrl(swaggerInfo.getLicenseUrl()).version(swaggerInfo.getVersion()).build();
    }
